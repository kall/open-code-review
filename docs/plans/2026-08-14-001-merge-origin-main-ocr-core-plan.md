...
- CI 게이트 계산식은 `go tool cover -func=coverage.out | grep total:`의 값이 90 미만이면 실패다(`ci.yml`).
  아래 수치는 모두 같은 계산식으로 측정했다.
- **기준선 실측치 A — origin/main(`5fafe82`) 단독, 2026-08-14 측정: 전체 91.2%, `cmd/opencodereview` 87.5%.**
  테스트 전건 통과(`go test -race -count=1 ./...` exit 0). 게이트까지 여유 **1.2%p**.
- **기준선 실측치 B — 병합 시뮬레이션 트리(수정 전): 전체 90.3%.**
  즉 병합 자체가 약 0.9%p를 깎아먹어 여유가 0.3%p로 줄어든다. Phase 3-11의 실질 합격선은 A가 아니라 B 근처다.
...
- Phase 3-11에서 병합 후 총계를 재측정하고 90%를 넘는지 확인한다. 목표는 91.2%(A) 회복이 아니라
  90% 게이트 통과이며, 90~90.5% 구간에 들어오면 여유가 없으므로 테스트를 추가로 보강한다.
